...
> Đây là file Claude Code luôn load. Tương đương `alwaysApply: true` của Cursor MDC.
...
| Khu vực | Cursor | Claude Code |
|---|---|---|
| Core rule (always) | `.cursor/rules/000-core.mdc` | File này (`CLAUDE.md`) |
| Rule theo folder | `.cursor/rules/*.mdc` (glob) | Nested `CLAUDE.md` trong folder + `docs/rules/*.md` |
| Subagent | (prompt template) | `.claude/agents/*.md` |
...
Mỗi agent file (`.claude/agents/<name>.md`) có YAML frontmatter (`name`, `description`, `tools`) và body chi tiết "Mục tiêu / Input / Output / Tài liệu cần đọc / Quy trình / Cấm".
