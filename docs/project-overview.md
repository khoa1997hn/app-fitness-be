...
- Yêu thích program: pivot `program_favorites` (user ↔ program, một user có thể yêu thích nhiều program). API `POST /api/v1/programs/favorites` body `program_ids` (thêm, không ghi đè favorite cũ, idempotent) + cờ `is_favorited` trên `GET /api/v1/programs` (Home).
