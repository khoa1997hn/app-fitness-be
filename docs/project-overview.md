...
- Yêu thích program: pivot `program_favorites` (user ↔ program). API `POST /api/v1/programs/{program}/favorite` (idempotent) + cờ `is_favorited` trên `GET /api/v1/programs` (Home).
