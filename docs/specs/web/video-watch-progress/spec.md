...
- **2026-06-01 — Refactor: bỏ `DB::table`/`DB::select`, quy hết về Model** → `VideoWatchProgressService` viết lại 100% bằng Eloquent: aggregate watched/completed qua `withSum`/`withCount` trên quan hệ mới `Video::progresses()`; "program đang học dở" (trước dùng window function `ROW_NUMBER() OVER` qua `DB::select`) đổi sang lấy progress sort `last_watched_at desc` rồi gom nhóm theo program trong PHP. `lessonProgress`/`programProgress` ủy quyền cho bản `...MapForUser`. Không đổi API/response. Kèm rule mới [`02-code-quality.md`](../../../rules/02-code-quality.md) cấm truy cập table trực tiếp.
