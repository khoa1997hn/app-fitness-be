    ...
    <div class="card">
        <div class="card-body flex flex-col p-6">
            <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                <div class="flex-1">
                    <div class="card-title text-slate-900 dark:text-white">Sửa tour</div>
                </div>
            </header>
            <form action="{{ route('admin.tours.update', $tour) }}" method="POST" class="card-text h-full space-y-4 max-w-4xl">
                @csrf
                @method('PUT')

                <div class="input-area">
                    <label for="name" class="form-label">Tên tour <span class="text-danger-500">*</span></label>
                    <input type="text" id="name" name="name" class="form-control" placeholder="Nhập tên tour" value="{{ old('name', $tour->name) }}" required>
                    @error('name')
                ...
                <div class="input-area">
                    ...
                    <input type="url" id="link" name="link" class="form-control" placeholder="Nhập link tour" value="{{ old('link', $tour->link) }}" required>
                    @error('link')
                        <span class="font-Inter text-sm text-danger-500 pt-2">{{ $message }}</span>
                    @enderror
                </div>
